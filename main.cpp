...
int main(int argc, char** argv) {
	...
	Model dino = offDino;
	Model vaca = offVaca;
	// models transformed into world coordinates
	Model tdilo = dilo * (Matrix(2) * Matrix().bank(180) * Matrix().head(180) * Matrix().translate(-0.5,0,1));
	...
	//
	Model camSpace = world * (Matrix().lookAt(Vector4(), Vector4(0, 0, 1)));

	Model screenSpace = camSpace * (Matrix().scale(1, -1, 1));

	Model target = screenSpace;

	float n = 1; //distance of the plane from the origin(center of projection)
	    //point rendering
